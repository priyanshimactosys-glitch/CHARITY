...
import UploadIconImg from "../../../../assets/icons/Icon.png"; 
...
const CalendarAddEvent = ({ isOpen, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-[110] flex items-center justify-center p-4">
      <div className="bg-white rounded-[24px] w-full max-w-[550px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-8 space-y-5 max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          {/* Title */}
          <div>
            <label className="block text-[11px] font-extrabold text-gray-400 mb-2 uppercase tracking-wider">
              Title
            </label>
          ...
          <div className="flex items-center gap-4 py-2 border-b border-gray-50">
            ...
            >
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${formData.isAllDay ? "border-[#0061AF] bg-[#0061AF]" : "border-gray-300"}`}
              >
                {formData.isAllDay && (
              ...
              <span
                className={`text-sm font-medium ${formData.isAllDay ? "text-gray-800" : "text-gray-400"}`}
              >
                All day Event
              </span>
            </button>
          ...
          {/* Visibility Level */}
          <div>
            <label className="block text-[11px] font-extrabold text-gray-400 mb-3 uppercase tracking-wider">
              Visibility Level
            </label>
            <div className="flex flex-wrap gap-x-6 gap-y-3">
              {["Volunteers", "Admin Only", "Personal", "All"].map((level) => (
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${formData.visibility === level ? "bg-[#4CAF50] border-[#4CAF50]" : "border-gray-300"}`}
                  >
                  ...
                  <span
                    className={`text-sm font-medium ${formData.visibility === level ? "text-black font-bold" : "text-gray-500"}`}
                  >
                    {level}
                  </span>
                </button>
  ...
  >
    {/* Lucide FileUp hata kar PNG icon lagaya */}
    <img src={UploadIconImg} alt="upload" className="w-5 h-5 object-contain" /> 
    Upload file
  </button>
</div>
          </div>
